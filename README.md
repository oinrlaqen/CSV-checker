...
Working in the US healthcare company, my primary goal is to <ins>implement client's file with the employees' insurance coverage info into the system</ins>.
  
As those files we typically receive may be messy from time to time, we need to validate them manually in Google Sheets. However, this process may be complicated and time consuming. When the file we are working with is large enough, it's also not a big deal to **miss some inconsistency** and load inappropriate or invalid info into the app.
